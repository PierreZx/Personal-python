...
def create_main_interface():
    ...
    global tab1, tab2, entry, entry1, entry2, check1_var, check2_var, check3_var, check4_var, data
    ...
    check1 = tk.Checkbutton(tab1, text="90/MÊS (3 VEZES NA SEMANA)", variable=check1_var)
    check1.grid(row=3, column=0, padx=5, pady=5, sticky="w")

    check2 = tk.Checkbutton(tab1, text="130/MÊS (5 VEZES NA SEMANA)", variable=check2_var)
    ...
    btn_salvar = ttk.Button(tab1, text="Salvar", command=lambda: [salvar_dados(), novo_cadastro(tab2)])
    btn_salvar.grid(row=8, column=0, columnspan=2, padx=10, pady=10)
...
btn_login.place(x=50,y=140)

btn_cadastro = ttk.Button(root, text="Cadastrar", command=cadastro)
btn_cadastro.place(x=170,y=140)
